...
def get_soo_stats(individuals, end):
    ...
    if end and not params['SILENT']:
        print_final_stats()
    if stats['gen'] <= 50:
            f = open("coco_small.txt",'a')
            for i in individuals:
                f.writelines(str(stats['gen'])+'\t')
                f.writelines(str(i.phenotype) + '\t')
                f.writelines(str(i.fitness) + '\n')
            # f.writelines(str(trackers.best_ever.fitness) + '\t')
            # f.writelines(str(trackers.worst_ever.phenotype) + '\t')
            # f.writelines(str(trackers.worst_ever.fitness) + '\t')
...
def update_stats(individuals, end):
    ...
    if stats['gen'] <= 50:
        ...
        f.close()
        # f = open("stats_eccv_final_5_32_.txt",'a')
        # for g in params['GENERATIONS']:
        #     for i in individuals:
        #         f.writelines(str(stats['gen'])+'\t')
        #         f.writelines(str(i.phenotype) + '\t')
        #         f.writelines(str(i.fitness) + '\n')
